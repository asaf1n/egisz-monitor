...
См. `k8s/README.md` и манифесты в `k8s/postgres/` (StatefulSet + PVC + Service `ClusterIP`). Секрет создаётся из `postgres-secret.example.yaml` локально (файл `postgres-secret.yaml` в `.gitignore`).
