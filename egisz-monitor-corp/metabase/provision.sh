...
if [ -x /app/setup-dashboards.sh ]; then
  ...
  SCHEMA_CHECK="$(PGPASSWORD="${APP_DB_PASSWORD}" psql -h "${PGHOST}" -U "${APP_DB_USER}" -d "${APP_DB_NAME}" -Atc "${SCHEMA_SQL}" 2>/dev/null | tr -d '[:space:]' || true)"
  SCHEMA_CHECK="${SCHEMA_CHECK:-0}"

  if [ -z "${SCHEMA_CHECK}" ] || ! [[ "${SCHEMA_CHECK}" =~ ^[0-9]+$ ]]; then
    SCHEMA_CHECK=0
  fi
